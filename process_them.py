...
from utils import create_dirs_for_file, PersistentList, hms, dhms
...
VIDEO = '-c:v libx265 -crf 26 -preset medium'
AUDIO = '-c:a aac -b:a 128k -ac 2'
FFMPEG_PARAMS = f'{VIDEO} {AUDIO} -tag:v hvc1 -movflags faststart'
...
def process_video(task: EncodingTask):
    ...
    out_moved_file = OUT_DIR / video_src.name
    ...
    print(f'Processing video: {video_src}, duration: {dur}s')
    out_file = TMP_OUT_DIR / video_src.name
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        while True:
            tasks = []
            que.reload()
            file_is_ok_to_process = lambda fn: (fn not in success.lines) and (fn not in errors.lines) and os.path.exists(fn)
            ...
            print(f'total_src_seconds: {dhms(total_src_seconds)}')
            futures = []
